...
namespace CarDealershipApp.Services
{
    public class FeatureService : IFeatureService
    {
        ...
        private readonly IRepository<CarFeature> carFeatureRepository;

        public FeatureService(IRepository<Feature> featureRepo, IRepository<CarFeature> carFeatureRepository)
        {
        ...
        {
            foreach (var carFeature in await carFeatureRepository
                .GetAllQueryable()
                .Where(x => x.FeatureId == id)
                .ToListAsync())
            {
                await carFeatureRepository.DeleteByConditionAsync(x => x.FeatureId == id);
            }
